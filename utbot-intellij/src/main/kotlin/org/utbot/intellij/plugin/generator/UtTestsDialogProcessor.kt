...
object UtTestsDialogProcessor {
                        ...
                        override fun run(indicator: ProgressIndicator) {
                            ...
                            for (srcClass in model.srcClasses) {
                                ...
                                withSubstitutionCondition(shouldSubstituteStatics) {
                                    ...
                                    val testCaseGenerator = UtBotTestCaseGenerator.apply {
                                        init(
                                            Paths.get(buildDir),
                                            classpath,
    ...
    private fun errorMessage(className: String?, timeout: Long) = buildString {
        appendln("UtBot failed to generate any test cases for class $className.")
        appendln()
        appendln("Try to alter test generation configuration, e.g. enable mocking and static mocking.")
        appendln("Alternatively, you could try to increase current timeout $timeout sec for generating tests in generation dialog.")
    }
